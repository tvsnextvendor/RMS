...
export class TraingClassTabComponent implements OnInit {
  ...
  getTrainingClassList() {
    ...
    let query = this.courseService.searchQuery(this.CMSFilterSearchEventSet) ? this.courseService.searchQuery(this.CMSFilterSearchEventSet) : (roleId != 1 ? '&createdBy=' + user.userId + '&resortId=' + resortId : '');

...
export class ResortChartsComponent implements OnInit {
  ...
  totalNoOfBadges(queries) {
    ...
    this.commonService.getBadges(query).subscribe((resp) => {
      ...
      // new Code
      var result = [];
      donutChartData.reduce(function (res, item) {
        if (!res[item.Badge.badgeName]) {
          res[item.Badge.badgeName] = { badgeName: item.Badge.badgeName, totalcount: 0 };
          result.push(res[item.Badge.badgeName])
        }
        res[item.Badge.badgeName].totalcount = parseInt(res[item.Badge.badgeName].totalcount) + parseInt(item.totalcount);
        return res;
      }, {});
      this.dashboardVar.totalNoOfBadges = result.map(item =>
        [item.badgeName, parseInt(item.totalcount, 10)]
      );
      //console.log(this.dashboardVar.totalNoOfBadges);
     // Old Code

...
const allRoles = rolessql.designations;
const mainUser = rolessql.user;
...
sequelize.sync({
	...
	// db.Designation.findAll().then(function (roleOutput) {
	// 	if (roleOutput.length === 0) {
	// 		db.Designation.bulkCreate(allRoles).then(function (rolescreated) {
	// 			if (rolescreated) {
					db.User.findAll().then(function (userOutput) {
						if (userOutput.length === 0) {
							db.User.create(mainUser).then(function (userCreated) {
